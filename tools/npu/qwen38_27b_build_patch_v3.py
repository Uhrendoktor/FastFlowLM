...
def ensure_peano()->Path:
    ...
    dist=importlib.metadata.distribution('llvm-aie'); base=Path(dist.locate_file('')); candidates=list(base.rglob('clang++')); pd=candidates[0].parent.parent if candidates else Path(dist.locate_file('llvm-aie'))
...
def patch_npu_build(nb:Path)->None:
    ...
    if '--no-xchesscc' not in s:s=s.replace(anchor,anchor+'''        *(["--no-xchesscc","--no-xbridge","--peano",os.environ["PEANO_INSTALL_DIR"]] if os.environ.get("QWEN38_USE_PEANO") == "1" else []),\n''',1)
    nb.write_text(s)
