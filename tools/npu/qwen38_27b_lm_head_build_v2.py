...
def main(root:Path)->int:
    ...
    if not peano_env: raise SystemExit('PEANO_INSTALL_DIR must point at the pinned toolchain LLVM/Peano install')
    peano=Path(peano_env); test=peano/'bin'/'clang++';
    if not test.is_file(): raise SystemExit(f'Peano clang++ missing: {test}')
    ...
    cmd=['make',f'M={M}',f'K={K}',f'N={N}','targetname=n1_core','aie_py_src=n1_core_bf16.py','use_chess=0']
